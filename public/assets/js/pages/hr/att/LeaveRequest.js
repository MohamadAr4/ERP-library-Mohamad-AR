...
let aApprove = PhSettings.PhsCodes.PhsApproveStatus;
jQuery(document).ready(function () {
  let options = {cols: 2, mode: PhF_Mode_Enter, type: PhF_Type_Form};
  let metta = {};
  let aURL = {};
  aURL.Url = PhSettings.apiURL;
  aURL.Api = '/UC/Emp/LeaveRequest';
  ...
  metta = {aURL: aURL, aFields: getFields(), aQFields: getQFields(), phTable: '', beforSave: checkValue, afterEdit: checkType};
  phForm = new PhForm('LeaveRequest', metta, options);
  $('#fldEmpName').focusout(function () {
    getVacations();
  });
  showHeaderSpinner(false);
  $('.eHour').addClass('d-none');
  $('.sHour').addClass('d-none');
  checkType();
});

function checkType() {
  $('#fldTypeId').on('change', function () {
    if ($('#fldTypeId').val() === "310" || $('#fldTypeId').val() === "210" || $('#fldTypeId').val() === "200") {
      $('.eHour').removeClass('d-none');
      $('.sHour').removeClass('d-none');
    } else {
      $('.eHour').addClass('d-none');
      $('.sHour').addClass('d-none');
    }
...
function getVacations() {
  ...
  if ($('#fldEmpId').val() > 0) {
    $.ajax({
      type: 'POST',
      async: false,
      url: PhSettings.apiURL + '/UC/Emp/Leaves/Query',
      headers: {
...
function checkValue() {
  ...
  if ($('#fldTypeId').val() === "300" || $('#fldTypeId').val() === "110" || $('#fldTypeId').val() === "100") {
