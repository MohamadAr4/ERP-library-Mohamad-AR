...
jQuery(document).ready(function () {
  ...
  aURL.Api = '/UC/Fix/ResponsibilityMaster';
  ...
  phForm = new PhForm('Responsibility', metta, options);
  $('#ph-Open').click(function (e) {
    ...
    $('#ResponsibilityFilter').removeClass('d-none');
    $('#ResponsibilityFilter').animate({
      height: 'toggle'
    });
  });
...
function toogle(show) {
  if (show) {
    $('#ResponsibilityFilterForm').removeClass("d-none");
    $('#ResponsibilityTable').addClass("d-none");
    $('#ResponsibilityFilterPager').addClass("d-none");
    $('#ph-dReset').removeClass("d-none");
    $('#ph-dExecute').html('<i class="bi bi-lightning"></i>');
    $('#ResponsibilityFilterTable').html('');
    bToogle = false;
  } else {
    $('#ResponsibilityFilterForm').addClass("d-none");
    $('#ResponsibilityTable').removeClass("d-none");
    $('#ResponsibilityFilterPager').removeClass("d-none");
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status && parseInt(response.code) === 200 && response.data.List !== []) {
        ...
        tableRowDataCount = 0;
        $('#ResponsibilityFilterTable').html(vHtml);
        $('#ResponsibilityFilterPager').addClass("d-none");
      }
    },
