...
jQuery(document).ready(function () {
  ...
  aURL.Api = '/UC/Cpy/DepartmentUnit';
  ...
  phForm = new PhForm('DepartmentUnits', metta, options);
    ...
    headers: {
      ...
      if (response.status) {
        for (let i = 0; i < response.data.List.length; i++) {
          aDepartment[i] = {};
          aDepartment[i].id = response.data.List[i].id;
          aDepartment[i].name = response.data.List[i].name;
        }
      }
    }
  });
