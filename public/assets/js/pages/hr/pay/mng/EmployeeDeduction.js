...
jQuery(document).ready(function () {
  ...
  aURL.Api = '/UC/Emp/EmployeeDeduction';
  ...
  phForm = new PhForm('EmployeeDeduction', metta, options);
  $('#fldDedId').change(function () {
    setAmount();
  });
  setAmount();
});
...
function getDeduction() {
  $.ajax({
    type: 'POST',
    async: false,
    url: PhSettings.apiURL + '/UC/Emp/Deduction/List',
    headers: {
      ...
      if (response.status) {
        for (let i = 0; i < response.data.List.length; i++) {
          aDeduction[i] = {};
          aDeduction[i].id = response.data.List[i].id;
          aDeduction[i].name = response.data.List[i].num + ' - ' + response.data.List[i].name;
          aDeduction[i].amt = response.data.List[i].amt;
          aDeduction[i].typeName = response.data.List[i].typeName;
        }
      }
    }
  });
