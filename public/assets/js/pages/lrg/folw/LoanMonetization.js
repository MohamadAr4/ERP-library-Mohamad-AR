...
jQuery(document).ready(function () {
  ...
  aURL.Api = '/UC/Lrg/LoanMonetization';
  ...
  phForm = new PhForm('LoanMonetization', metta, options);
});
...
function getClaim() {
  $.ajax({
    type: 'POST',
    async: false,
    url: PhSettings.apiURL + '/UC/Lrg/ReqFinancialClaim/List',
    headers: PhSettings.Headers,
    success: function (response) {
      if (response.status) {
        for (let i = 0; i < response.data.List.length; i++) {
          aClaim[i] = {};
          aClaim[i].reqId = response.data.List[i].reqId;
