...
jQuery(document).ready(function () {
  ...
  phForm = new PhForm('Output', metta, options);
  ...
  $('#ph-Open').click(function (e) {
    ...
    $('#OutputFilter').removeClass('d-none');
    $('#OutputFilter').animate({
      height: 'toggle'
    });
  });
...
function onChangePrice() {
  ...
  let nPrice = parseFloat(phForm.phTable.phT0.getFieldValue(nRow, 'price'));
...
function toogle(show) {
  if (show) {
    $('#OutputFilterForm').removeClass("d-none");
    $('#OutputTable').addClass("d-none");
    $('#OutputFilterPager').addClass("d-none");
    $('#ph-dReset').removeClass("d-none");
    $('#ph-dExecute').html('<i class="bi bi-lightning"></i>');
    bToogle = false;
  } else {
    $('#OutputFilterForm').addClass("d-none");
    $('#OutputTable').removeClass("d-none");
    $('#OutputFilterPager').removeClass("d-none");
...
function getQueryData() {
  ...
  if (phForm.phTable.phT0.getRowCount() > 0) {
    ...
    for (let j = 0; j < phForm.phTable.phT0.getRowCount(); j++) {
      aQData[idx].values[j] = phForm.phTable.phT0.getFieldValue(j, 'trnId');
    }
  }
  return aQData;
}
...
function drawData(nIndex) {
  ...
  phForm.phTable.phT0.setFieldValue(nRow, 'costId', 0);
  phForm.phTable.phT0.setFieldValue(nRow, 'ord', aData[nIndex].trnOrd);
  phForm.phTable.phT0.setFieldValue(nRow, 'trnId', aData[nIndex].trnId);
  ...
  phForm.phTable.phT0.setFieldValue(nRow, 'qnt', aData[nIndex].trnQnt);
  phForm.phTable.phT0.setFieldValue(nRow, 'price', aData[nIndex].trnPrice);
  phForm.phTable.phT0.setFieldValue(nRow, 'amt', aData[nIndex].trnAmt);//Tot_Amt
  phForm.phTable.phT0.setFieldValue(nRow, 'rem', '');
  phForm.phTable.phT0.setFieldValue(nRow, 'famt', aData[nIndex].trnFamt);
  phForm.phTable.phT0.setFieldValue(nRow, 'ftot', aData[nIndex].trnFtot);
  phForm.phTable.phT0.setFieldValue(nRow, 'ramt', aData[nIndex].trnRamt);
  phForm.phTable.phT0.setFieldValue(nRow, 'rtot', aData[nIndex].trnRtot);
  phForm.phTable.phT0.setFieldValue(nRow, 'perc', aData[nIndex].trnPerc);
  phForm.phTable.phT0.setFieldValue(nRow, 'cst', aData[nIndex].trnPrice);
  phForm.phTable.phT0.setFieldValue(nRow, 'ucst', aData[nIndex].trnPrice);
  phForm.phTable.phT0.render();
}

function drawDeleteRow(nId) {
  let nRow = phForm.phTable.phT0.getRowNum('trnId', nId);
