...
import { meta } from "../../../data/Jsons/str/qry/ItemMovementsCard";
import axios from "axios";
function ItemMovementsCard() {
  ...
  const Atype = useSelector(
    (state) => state.user.user.data.UsrCodes.StrTransactionType
  );
  const aTrnType = Atype.map((setting) => ({
    value: setting.id,
    label: setting.name,
  }));
